//THIS IS THE ARRAY OF THE POEM OR PROSE
//IT SHOULD ALWAYS START "poem = "
...
function randomLines(numberLines) {
	changedText += "<B>" + numberLines + " RANDOM LINES</B><BR>\n"
	for (var i = 0; i < numberLines; i++) {
		var rand = Math.floor(Math.random() * (textArray.length - 0)) + 0;
		changedText += textArray[rand] + "<BR>\n";
...
//UNLESS THERE ISN'T ANYMORE WORDS IN THE LINE OR SENTENCE
function phasesThatBeginWith(myWord) {
changedText += "<B>THREE WORD PHRASE RESULTS</B><BR>\n"
 for (i in textArray) {
 	if (textArray[i].length > 1) {
 		var myLine = textArray[i].split(" ");
 		 for (var j = 0; j < myLine.length; j++) {
 			var cleanWord = myLine[j].replace(/[^a-z0-9]/gi, '');
 		 	if (cleanWord.toLowerCase() == myWord) {
 		 		if (j < myLine.length - 2) {
 		 			changedText += myLine[j] + " " + myLine[j+1] + " " + myLine[j+2] + "<BR>";
 		 		}
...
function sortByLineLenght(theOrder) {
 ...
 for (i in textArray) {
 	if (textArray[i].length > 1) {
	items.push({key: textArray[i], value: textArray[i].length})
}
 }
 if (theOrder == "longest") {
...
function lastwordAlpha(rhymeWord) {
 ...
 for (i in textArray) {
 	if (textArray[i].length > 1) {
 		   ...
 		   var n = textArray[i].trim().split(" ");
 		   if (rhymeWord) {
 		   var lastWord = n[n.length - 1]; 	
 		   } else {
 		   var lastWord = n[0]; 	
 		   }
 	items.push({key: textArray[i], value: lastWord.toLowerCase()});
}
 }
items.sort(function(a, b){
...
function displayLines(numberLines) {
	if (numberLines == "ALL") {
		numberLines = textArray.length;
	}
	changedText += "<BR><B>RESULTS FOR SORT FUNCTIONS</B><BR>\n"
for (var i = 0; i < numberLines;i++) {
changedText += textArray[i] + "<BR>\n";
}
